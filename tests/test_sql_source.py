"""
Tests for XArraySQLSource — the SQL-backed xarray source.
...
- SQL transforms (filter, limit, aggregation)
- Dimension info API
...
class TestConstruction:
    ...
    def test_from_air_temperature(self, air_dataset):
        source = XArraySQLSource(_dataset=air_dataset)
        assert source.get_tables() == ["air"]
    ...
    def test_repr(self, synthetic_dataset):
        ...
        assert "temperature" in r or "pressure" in r
...
class TestSQLExecution:
    ...
    def test_air_temperature_sql(self, air_dataset):
        source = XArraySQLSource(_dataset=air_dataset)
        df = source.execute(
            "SELECT lat, AVG(air) as avg_air FROM air GROUP BY lat ORDER BY lat LIMIT 5"
        )
        assert len(df) == 5
...
class TestSchemaMetadata:
    ...
    def test_get_metadata(self, synthetic_dataset):
        ...
        # Single table: @cached_metadata unwraps to metadata[table]
...
class TestEstimateSize:
    ...
    def test_single_table(self, synthetic_dataset):
        ...
        assert "exceeds_warning" in info
...
class TestSQLTransformsInGet:
    ...
    def test_get_with_sql_transforms_empty(self, synthetic_dataset):
        source = XArraySQLSource(_dataset=synthetic_dataset)
        df = source.get("temperature", sql_transforms=[])
        assert len(df) > 0
